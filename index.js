...
function printIndexes(array) {
  for (const point in array){console.log(point)};
  return ;
...
function findMinMax([firstElement, ...restElements]) {
  let min = firstElement, max =firstElement ;
  for (const x of restElements)
  {x > max ? max=x : x < min ? min = x :`continue`  };
...
function printStrings(array) {
  return console.log ((array.toString()).repeat(2));
...
function addTenToEach(array) {
  const newArr = Array(array.length);
  for (const index in array){
 
    newArr[index] = array[index] + 10;
  }
...
function swapMinMax(array) {
  ...
  for (let i = 1; i<array.length ;i++){
    ...
    if(array[i] < max) {min=array[i]; minIndex=i;}
  }
...
function replaceNumbers(arr) {
  ...
  let y = 0;
  for ( const i of arr){
    i > 10 ? nArr[y]="Greater than 10":  nArr[y]="Less than or equal to 10";
    y++;
  }
  return nArr;
  // Створюємо пустий масив для зберігання результату
